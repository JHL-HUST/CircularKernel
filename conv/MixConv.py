...
class MixConv(CircleLayerBase):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1,
        ...
        super(MixConv, self).__init__(in_channels, out_channels, kernel_size, stride, padding, dilation,
...
class Mix3rConv(CircleLayerBase):
    """
        Input  [b, in_c,  h, w]
        Output [b, out_c, h, w]
        Note: Kernel_size must be 3 at present
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1,
                 padding=0, dilation=1, groups=1,
                 bias=True, padding_mode='zeros', version='MixConv', probs=[0.25,0.25,0.25]):
        super(Mix3rConv, self).__init__(in_channels, out_channels, kernel_size, stride, padding, dilation,
                                              groups, bias, padding_mode, version)
        if self.kernel_size != 3 and self.kernel_size != 1:
            print("Kernel_size must be 1 or 3, %d was given" % kernel_size)
            raise NotImplemented("Kernel_size must be 1 or 3")
        ...
        if kernel_size == 3:
            ...
            self.weight_dim = 1
            self.dense_count = 1
        self.weight = Parameter(torch.zeros(out_channels, in_channels, self.kernel_size, self.kernel_size))

    def build_transforms(self, probstr):
        Ts = []
        Ts.append(self.get_w_transform_matrix(*self.init_bilinear_weights_L0()))
        # print("dist_to_center = %d, " % 0, Ts[-1])
        Ts.append(self.get_w_transform_matrix(*self.init_bilinear_weights((np.sqrt(2)+1) / 2)))
        # print("dist_to_center = %d, " % 0, Ts[-1])
        Ts.append(self.get_w_transform_matrix(*self.init_bilinear_weights()))
        # print(Ts[-1])
        Ts.append(self.get_w_transform_matrix(*self.init_bilinear_weights(np.sqrt(2))))
...
class WMixConv(CircleLayerBase):
    """
        Input  [b, in_c,  h, w]
        Output [b, out_c, h, w]
        Note: Kernel_size must be 3 at present
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1,
                 padding=0, dilation=1, groups=1,
                 bias=True, padding_mode='zeros', version='MixConv', probs=[0.25,0.25,0.25]):
        super(WMixConv, self).__init__(in_channels, out_channels, kernel_size, stride, padding, dilation,
                                              groups, bias, padding_mode, version)
        if self.kernel_size != 3 and self.kernel_size != 1:
            print("Kernel_size must be 1 or 3, %d was given" % kernel_size)
            raise NotImplemented("Kernel_size must be 1 or 3")
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.kernel_size != 1:
            ...
            #import code
            #code.interact(local=locals())
            w = w.matmul(torch.transpose(T,0,1))
        w = w.view(w_size[0], w_size[1], self.kernel_size, self.kernel_size)
        # if sum(self.cnts) % 200 == 0:
        #     print(self.probs, [cnt / sum(self.cnts) for cnt in self.cnts] )
        return nn.functional.conv2d(x, w, self.bias, self.stride, self.padding, self.dilation)
...
class SmoothMixConv(MixConv):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1,
                 padding=0, dilation=1, groups=1,
                 bias=True, padding_mode='zeros', version='SmoothMixConv', probs=None, smooth=100):
        super(SmoothMixConv, self).__init__(in_channels, out_channels, kernel_size, stride, padding, dilation,
                                              groups, bias, padding_mode, version, probs=probs)
        self.choice = 0
        self.iter_cnt = 0
        self.smooth_iter = smooth
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.kernel_size != 1:
            if self.training:
                if self.iter_cnt % self.smooth_iter == 0:
                    self.choice = np.random.choice(self.length_T, p=self.probs)
