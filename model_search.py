  ...
  def __init__(self, steps, multiplier, C_prev_prev, C_prev, C, reduction, reduction_prev, PRIMITIVES):
    ...
    for i in range(self._steps):
      for j in range(2+i):
        ...
        op = MixedOp(C, stride, PRIMITIVES) #op是构建两个节点之间的混合
...
class Network(nn.Module):
  ...
  def __init__(self, C, num_classes, layers, criterion, operation, steps=4, multiplier=4, stem_multiplier=3):
    ...
    for i in range(layers):
      ...
      cell = Cell(steps, multiplier, C_prev_prev, C_prev, C_curr, reduction,
                  reduction_prev, self.PRIMITIVES)  # 每个cell的input nodes是前前cell和前一个cell的输出
  ...
  def new(self):
    model_new = Network(self._C, self._num_classes, self._layers, self._criterion, self.operation).cuda()
  ...
  def genotype(self):
          ...
          for k in range(len(W[j])):
            if k != self.PRIMITIVES.index('none'):
              if k_best is None or W[j][k] > W[j][k_best]:
                k_best = k
          gene.append((self.PRIMITIVES[k_best], j))#把(操作，前驱节点序号)放到list gene中，[('sep_conv_3x3', 1),...,]
        start = end
        n += 1
    ...
    n = 3
    start = 2
    weightsr2 = F.softmax(self.betas_reduce[0:2], dim=-1)  # 0,1
    weightsn2 = F.softmax(self.betas_normal[0:2], dim=-1)
    for i in range(self._steps-1):
      end = start + n  #5
      tw2 = F.softmax(self.betas_reduce[start:end], dim=-1)  # [2,5]
    ...
    gene_normal = _parse(F.softmax(self.alphas_normal, dim=-1).data.cpu().numpy(),weightsn2.data.cpu().numpy())#得到normal cell 的最后选出来的结果
    gene_reduce = _parse(F.softmax(self.alphas_reduce, dim=-1).data.cpu().numpy(),weightsr2.data.cpu().numpy())#得到reduce cell 的最后选出来的结果
