    ...
    def __init__(self, C, num_classes, layers, criterion, operation, steps=4, multiplier=4, stem_multiplier=3):
        ...
        for i in range(layers):
            if i in [layers // 3, 2 * layers // 3]:
                ...
                reduction = False
            cell = Cell(steps, multiplier, C_prev_prev, C_prev, C_curr, reduction, reduction_prev, self.PRIMITIVES)
        ...
        def _parse(weights, weights2):
            ...
            for i in range(self._steps):
                ...
                edges = sorted(range(i + 2), key=lambda x: -max(
                    W[x][k] for k in range(len(W[x])) if k != self.PRIMITIVES.index('none')))[:2]
